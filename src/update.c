...
//====Prototipos====//
void update_jugando(s_GameState *gs, s_InputState *input, s_Assets *assets);
void update_tiempo_jugado(s_GameState* gs);
void update_levi_movimiento(s_GameState *gs, s_InputState *input, s_Assets *assets);
void valida_levi_suelo(s_Assets *assets, s_GameState *gs);
void transicion_pantalla(s_GameState *gs, s_InputState *input, s_Assets *assets);
...
void update(s_GameState *gs, s_InputState *input, s_Assets *assets)
{
    switch(gs->estadoPantalla)
    {
        ...
        case PANTALLA_JUGANDO:
            update_jugando(gs,input,assets);
            break;
...
void update_jugando(s_GameState *gs, s_InputState *input, s_Assets *assets)
{
    ...
    update_levi_movimiento(gs, input, assets);
...
void update_levi_movimiento(s_GameState *gs, s_InputState *input, s_Assets *assets)
{
    ...
    if(input->keyA == 1) //Camina izquierda
    gs->levi.x -= 3;
    if(input->keyD == 1) //Camina derecha
    gs->levi.x += 3;
    /*if(input->keyW == 1)
    gs->levi.y -= 2;
    if(input->keyS == 1) 
    ...
    //====Doble salto====//
    if(input->keySpace == 1 && gs->levi.y == SCREEN_Y - 66 - al_get_bitmap_height(assets->levi.levi_parado)*2) //Salto
        {
    ...
    if(input->keySpace == 1 && gs->levi.doble_salto == 1)
    {
        gs->levi.velocidadY = -8;
        gs->levi.doble_salto = 0;
    }

    if(input->keyLShift == 1 && input->keyD == 1)
    {
        printf("LShift presionado");
    }
    ...
    transicion_pantalla(gs, input, assets);
